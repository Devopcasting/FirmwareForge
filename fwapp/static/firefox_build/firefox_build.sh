...
latest_firefox_url=$(curl -s 'https://download.mozilla.rg/?product=firefox-latest&os=linux64&lang=en-US'| grep -oP 'https://[^"]+\.tar\.bz2')
...
cp -pa $FIREFOX_BUILD_PATH/references/distribution/* "$FIREFOX_BUILD_PATH/$BUILD_ID/tmp/$firefox_pkg_name/opt/firefox/distribution/"
cp -pa $FIREFOX_BUILD_PATH/references/defaults/* "$FIREFOX_BUILD_PATH/$BUILD_ID/tmp/$firefox_pkg_name/opt/firefox/defaults/"
cp -pa $FIREFOX_BUILD_PATH/references/usr/* "$FIREFOX_BUILD_PATH/$BUILD_ID/tmp/$firefox_pkg_name/usr/"
